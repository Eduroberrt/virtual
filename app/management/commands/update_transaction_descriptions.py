...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(
                    f'DRY RUN: Would update {total_count} transaction descriptions'
                )
            )
        ...
        with transaction.atomic():
            for tx in transactions_to_update:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f'Error updating transaction {tx.id}: {str(e)}'
                        )
                    )
    ...
    def update_description(self, description):
        ...
        updated = re.sub(
            r'\s*\([Dd]aisy\s+[Ss][Mm][Ss]\s+expired\)',
            '',
            updated
        )
        updated = re.sub(
            r'\s*\([Dd]aisy\s+[Ss][Mm][Ss]\s+cancelled\)',
            '',
            updated
        )
        updated = re.sub(
            r'\s*\([Dd]aisy\s+sms\s+expired\)',
            '',
            updated
        )
        updated = re.sub(
            r'\s*\([Dd]aisy\s+sms\s+cancelled\)',
            '',
            updated
        )
